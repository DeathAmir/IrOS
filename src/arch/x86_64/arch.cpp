...
void pic_init(){
    io_out8(0x20,0x11);io_out8(0xA0,0x11);
    io_out8(0x21,0x20);io_out8(0xA1,0x28);
    io_out8(0x21,4);io_out8(0xA1,2);
    io_out8(0x21,1);io_out8(0xA1,1);
    io_out8(0x21,0xFF);io_out8(0xA1,0xFF);
}
void pic_eoi(uint8_t v){if(v<32||v>47)return;if(v>=40)io_out8(0xA0,0x20);io_out8(0x20,0x20);}
void pit_init(uint32_t hz){if(hz<19)hz=19;uint32_t d=1193182u/hz;io_out8(0x43,0x36);io_out8(0x40,d&0xFF);io_out8(0x40,(d>>8)&0xFF);}
uint64_t ticks(){uint32_t lo,hi;asm volatile("rdtsc":"=a"(lo),"=d"(hi));return ((((uint64_t)hi)<<32)|lo)/10000000ull;}
...
extern "C" void interrupt_dispatch(InterruptFrame* f){
    ...
    if(v<32){char h[19];ir::u64hex(v,h);arch::serial_print("IrOS exception ");arch::serial_print(h);arch::serial_print(" RIP=");ir::u64hex(f->rip,h);arch::serial_print(h);arch::serial_print(" ERR=");ir::u64hex(f->error,h);arch::serial_print(h);if(v==14){arch::serial_print(" CR2=");ir::u64hex(cpu_read_cr2(),h);arch::serial_print(h);}arch::serial_print("\n");cpu_cli();for(;;)asm volatile("hlt");}
    arch::pic_eoi(v);
}
