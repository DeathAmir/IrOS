...
void keyboard_irq(){
    uint8_t s=io_in8(0x60); bool released=(s&0x80)!=0; uint8_t code=s&0x7F;
...
bool key_pop(KeyEvent& e){cpu_cli();if(qt==qh){cpu_sti();return false;}e=queue[qt];qt=(qt+1)&127;cpu_sti();return true;}
...
void mouse_init(int width,int height){
    ...
    wait_write();io_out8(0x64,0x20);wait_read();uint8_t status=io_in8(0x60)|2;
...
void mouse_irq(){
    uint8_t b=io_in8(0x60);if(pindex==0&&(b&0x08)==0)return;packet[pindex++]=b;if(pindex<3)return;pindex=0;
    int dx=(int8_t)packet[1],dy=(int8_t)packet[2];if(packet[0]&0xC0)return;
    ms.dx=dx;ms.dy=-dy;ms.x+=dx;ms.y-=dy;if(ms.x<0)ms.x=0;if(ms.y<0)ms.y=0;if(ms.x>=mw)ms.x=mw-1;if(ms.y>=mh)ms.y=mh-1;ms.buttons=packet[0]&7;ms.changed=true;
}
